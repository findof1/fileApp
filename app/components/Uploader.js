...
const Uploader = ({ userdata }) => {
            ...
            if (customName.length < 60) {
              if (allowedWord(customName)) {
                if (allowedWord(description)) {
                  ...
                  setErrMsg("Your userdata cookie is corrupted, please refresh, click home, and log back in")
                }
                } else {
  ...
  return (
    <div class="flex flex-col items-center justify-center">
      ...
      <TextInput
        label="Set A Name For Your File: "
        extraStyles="mt-12"
        onChange={(e) => {
      ...
      ></TextInput>
      <TextArea
        label="Set A Description For Your File: "
        extraStyles="mt-12"
        onChange={(e) => {
      ...
      <p>{errMsg}</p>
      <Button style="submit" onClick={saveFile} extraStyles="mt-4 w-28 h-12">
        Submit
      </Button>
    </div>
  );
